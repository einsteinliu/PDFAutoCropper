...
for file in files:
    ...
    if(abs(bestAngle) > 0.2):
        correctedImage = transform.rotate(oriImage,-1.0 * bestAngle)
        bd = int(len * numpy.tan(abs(bestAngle * numpy.pi / 180.0))) + 1
        correctedImage[0:bd,0:correctedImage.shape[1] - 1] = 1
        correctedImage[correctedImage.shape[0] - bd:correctedImage.shape[0] - 1,0:correctedImage.shape[1] - 1] = 1
        correctedImage[0:correctedImage.shape[0] - 1,0:bd] = 1
        correctedImage[0:correctedImage.shape[0] - 1,correctedImage.shape[1] - bd:correctedImage.shape[1] - 1] = 1
        io.imsave(file,correctedImage)
